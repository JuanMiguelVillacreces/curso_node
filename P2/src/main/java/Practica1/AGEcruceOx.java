...
import Practica1.GreedyAleatorizado;
 ...
 * @author Juanmy
 */
public class AGEcruceOx {

    private Ramdom rand;

    private StringBuilder log;

    public AGEcruceOx(long _semilla) {
        rand = new Ramdom();
    ...
    void cruceox(float[][] flu, float[][] loc, int tam, int ncromosomas, int ncromosomasaleatorios, long semilla, float probmuta, int evaluaciones, int[] s, int iteraciones) {

        log.append("INICIO EJECUCION: Algoritmo AGEcruceOX\n");
        ...
        log.append("Coste Final AGEcruceOX: " + mejorCo + "\n");
        log.append("Solucion AGEcruceOX:" + "\n");
        for (int i = 0; i < tam; i++) {
            log.append(" - AGEcruceOX[" + i + "] = " + s[i] + "\n");
        }
    ...
    void Ox(int[] a, int[] b) {
        ...
        p1 = rand.Randint(0, tam - 1);
        //para que no sean igual posicion de corte
        ...
        int ins2 = p2 + 1;
        for (int i = 0; i < tam; i++) {
            if (!busca(a, b[i], p1, p2, p)) {
                r1[ins % tam] = b[i];
                ins++;
            }
            if (!busca(b, a[i], p1, p2, p)) {
                r2[ins2 % tam] = a[i];
                ins2++;
            }
        }
